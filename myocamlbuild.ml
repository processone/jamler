(* OASIS_START *)
(* DO NOT EDIT (digest: b37815181bdd9ad02fcbd44c11fe7dc6) *)
     ...
     flags =
       [
          (["oasis_executable_jamler_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-lexpat"])]);
          (["oasis_executable_jamler_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lexpat"])]);
          (["oasis_executable_jamler_native"; "ocaml"; "link"; "native"],
       ...
       match hook with
         | After_rules ->
             List.iter
               (fun m ->
                  let tag = "pa_" ^ m in
		  let file = "src" / "pa_" ^ m -.- "cmo" in
                    flag ["ocaml"; "compile"; tag] & S[A "-ppopt"; A file];
                    flag ["ocaml"; "ocamldep"; tag] & S[A "-ppopt"; A file];
                    flag ["ocaml"; "doc"; tag] & S[A "-ppopt"; A file];
                    dep ["ocaml"; "ocamldep"; tag] [file]
	       ) ["jlib"; "sql"];
	     let plugins = string_list_of_file "src/plugins.list" in
	     let plugins_mod =
	       List.map String.capitalize plugins
	     in
	     (*let plugins_byte =
	       List.map (fun s -> "src" / s -.- "cmo") plugins
	     in
	     let plugins_native =
	       List.map (fun s -> "src" / s -.- "cmx") plugins
	     in
	     let plugins_obj =
	       List.map (fun s -> "src" / s -.- "o") plugins
	     in*)
		 ...
		 S (List.map (fun f -> P f) plugins_native);
	       ...
	       dep ["ocaml"; "program"; "native"; "use_plugins"] plugins_obj;
	       (*dep ["ocaml"; "compile"; "native"; "use_plugins"] plugins_obj;*)
	     ...
	     (*use_lib "src/main" "src/mod_roster"*)
	     (*rule "bla" ~prod:"src/main.native" ~dep:"src/mod_roster.o"
	       ~insert:`bottom
	       (fun _ _ -> Command.Nop)*)
	       (*rule "jamler main"
		 ...
		 (fun env build ->
		    Ocamlbuild_pack.Ocaml_compiler.native_link
		      "%.cmx" "%.native"
		 )*)
	     let plugins_lines =
	       List.map
	     ...
	     in
	     rule "bla" ~prod:"src/plugins.ml"
	       (fun _ _ ->
		  Echo (plugins_lines, "src/plugins.ml"))
