...
struct
  ...
  let src = Jamler_log.new_src "c2s"
  ...
  let process_unauthenticated_stanza state el =
    ...
    match Jlib.iq_query_info el with
    | `IQ iq -> (
      match Hooks.run_fold c2s_unauthenticated_iq
	 ...
	 let iq = {iq with
		    Jlib.iq_type =
                      `Error (Jlib.err_service_unavailable, None)}
	 in
	 let res =
  ...
  let process_privacy_iq from to' iq state =
	...
	match (Hooks.run_fold Privacy.privacy_iq_set state.server
		 (`Error Jlib.err_feature_not_implemented)
		 (from, to', iq)) with
  ...
  let wait_for_auth msg state =
	...
	| Some jid when (Jamler_acl.match_rule
	  ...
	  in
	  match Auth.check_password_digest_with_authmodule
		  jid.Jlib.luser state.server p d dgen with
	  | Some _auth_module -> (
	    (*?INFO_MSG(
	      ...
	      jlib:jid_to_string(JID), AuthModule]),*)
            Logs.info ~src
	      (fun m ->
                m "%a accepted legacy authentication for %S"
		  pp_pid state.pid
	    ...
	    | None ->
		 ...
		 Jlib.jid_tolower (Jlib.jid_remove_resource jid)
		   ...
		   pres_f = LJIDSet.of_list fs;
		   pres_t = LJIDSet.of_list ts;
	  ...
	  | _ ->
             Logs.info ~src
	       (fun m ->
                 m "%a failed legacy authentication for %S"
		   pp_pid state.pid
		   (Jlib.jid_to_string jid));
	     let err =
	...
	| None ->
           Logs.info ~src
	     (fun m ->
               m "%a forbidden legacy authentication for %S with resource %S"
		 pp_pid state.pid
		 u r);
	...
	| Some jid ->
	   (*?INFO_MSG(
	     ...
	     jlib:jid_to_string(JID)]),*)
           Logs.info ~src
	     (fun m ->
               m "%a forbidden legacy authentication for %S"
		 pp_pid state.pid
		 (Jlib.jid_to_string jid));
  ...
  let wait_for_feature_request msg state =
	...
	| SASL.Done props -> (
	  ...
	  (*?INFO_MSG("(~w) Accepted authentication for ~s by ~p",
          ...
          Logs.info ~src
	    (fun m ->
              m "%a accepted authentication for %S"
		pp_pid state.pid
		(u :> string));
          send_element state
            (`XmlElement ("success",
	...
	| SASL.ErrorUser (error, username) ->
	   (*?INFO_MSG(
	     ...
	     Username, StateData#state.server]),*)
           Logs.info ~src
	     (fun m ->
               m "%a failed authentication for %s@%s"
		 pp_pid state.pid
	...
	| SASL.Error error ->
	 ...
	 Eio.Cancel.cancel state.receiver Stop;
	 XMLReceiver.reset_stream state.xml_receiver;
	 send_element
	   state
  ...
  let wait_for_sasl_response msg sasl_state state =
	...
	| SASL.ErrorUser (error, username) ->
           Logs.info ~src
	     (fun m ->
               m "%a failed authentication for %S at %s"
		 pp_pid state.pid
		 username (state.server :> string));
	   send_element state
	     (`XmlElement
