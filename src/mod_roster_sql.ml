...
struct
  ...
  let read_roster user server =
    try
      let items = get_roster_query server user in
      let jid_groups = get_roster_jid_groups_query server user in
    ...
    let update_user =
      [%sql {|
	     ...
	     where username = %(username)s and jid = %(sjid)s
             |}]
    in
    Sql.update_t insert_user update_user;
    let delete_groups =
      [%sql {|
	     delete from rostergroups
             ...
             |}]
    in
    let _ = Sql.query_t delete_groups in
    List.iter
      (fun group ->
	let insert_group =
	  [%sql {|
	         ...
	         |}]
	in
	let _ = Sql.query_t insert_group in
	()
      ) item.groups
    ...
    let query2 =
      [%sql {|
	     delete from rostergroups
             ...
             |}]
    in
    let _ = Sql.query_t query1 in
    let _ = Sql.query_t query2 in
    ()
  ...
  let item_set_transaction' luser _lserver jid1 attrs els =
    ...
    let res = read_roster_item_t luser ljid in
    let item' =
      match res with
	| [] ->
	    {jid = jid;
  ...
  let subscription_transaction' direction luser _lserver jid1 type' reason =
    let ljid = Jlib.jid_tolower jid1 in
    let item =
      match read_roster_item_t luser ljid with
      | [] ->
	 let jid =
	 ...
	 {jid;
      ...
      | [item] ->
	 let r = raw_to_record item in
	 let groups = get_roster_groups_query_t luser ljid in
	 {r with groups}
      | _ -> assert false
    in
    let new_state =
    ...
    | Some subscription ->
	 ...
	 {item with
       ...
       in
       write_roster_item_t luser ljid new_item;
       (*case roster_version_on_db(LServer) of
	 true -> mnesia:write(#roster_version{us = {LUser, LServer}, version = sha:sha(term_to_binary(now()))});
	 false -> ok
	 end,*)
